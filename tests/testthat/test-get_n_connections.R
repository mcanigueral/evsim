...
test_that("the number of connections is calcualted properly by Session", {
  n_connections <- sessions %>%
    get_n_connections(by = "Session", resolution = 15)
  expect_true(any(names(n_connections) %in% sessions$Session))
})

test_that("the number of connections is calcualted properly by Profile", {
  n_connections <- sessions %>%
    mutate(Profile = "All") %>%
...
test_that("the number of connections is calcualted properly with custom datetime sequence", {
  dttm_seq <- seq.POSIXt(
